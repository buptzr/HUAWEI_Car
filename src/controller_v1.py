...
class controller:
    ...
    def passing_cross(self, road_id,cross_id,direction, car_id,next_road_id):#汽车通过路口，修改相关参数,返回1说明调度成功，返回-1说明调度失败
        ...
        if flag >= 2:#变换了道路，就将新的道路添加到path
            thiscar.path.append(thiscar.road_id)
        if flag == 0:
    ...
    def go_to_road(self, cross_id):
        ...
        if flag == 1:
            ...
            self.map.on_road += 1
            start_car.realstarttime = self.map.current_time
            start_car.path.append(start_car.road_id)
        return flag
    ...
    def main(self):
        ...
        for thiscar_id in self.map.carlist:
            ...
            temp = '('+str(thiscar_id)+','+str(thiscar.realstarttime)
            for item in thiscar.path[:-1]:
                temp += (','+str(item))
            temp += ')\n'
            self.map.result += temp
